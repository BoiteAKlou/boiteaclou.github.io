...
* The first step in the encryption process is to assign an integer in the range 0 to m-1 to each letter, where **m is the size of the alphabet** (usually 26).
...
To give you an example, let's say we want to encrypt the following sentence using the key defined earlier:
...
The affine cipher often uses an alphabet of 26 letters including 12 numbers which are coprime with 26. This gives us a total of 12 x 26 = **312 possible keys**.

This is something we can easily bruteforce so it is considered as highly insecure in the light of Kerckhoffs' Principle.
...
For example, if the most occurent letter in a ciphered text is "Z", you can suppose "Z" replaces the most common letter of the source language of the plain text ("E" in the English language).
...
The first two equations will be enough for us to find the key. It gives us the following system:

$$
\left\{
\begin{array}{r c l}
0a + b &\equiv& 12\mod{26}\\
...
That's it for this challenge but don't worry, it was only the warm-up! ;-) Do not hesitate to leave a comment or to contact me for any question or inquiry.
